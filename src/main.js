...
async function init() {
    return await setupXR('immersive-ar');
}
...
export let group;
export let renderer;
export let lastCirclePosition;
export let animation_camera;
export let interface_intro;
init().then((xrSession) => {
    ...
    const xrButton = XRButton.createButton(renderer, {requiredFeatures: ['hit-test', 'light-estimation']});
